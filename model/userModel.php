...
{
	$current_time = time();
	$subject = "📥 insta42 : activate your account"."\n";
	$link = "http://".$_SERVER['HTTP_HOST']."/insta42/";
	$link .='activate.php?log='.urlencode($login).'&key='.urlencode($activation_key).'&time='.urlencode($current_time);

	$message = "Welcome on insta42 !"."\n\n";
...
{
	$current_time = time();
	$subject = "📥 insta42 : Reset your password"."\n";
	$link = "http://".$_SERVER['HTTP_HOST']."/insta42/";
...
function notif_mail($id, $writer, $comment) {
	...
	if (verify_notification($id_author) == true)
	{
		...
		$mail = $data->mail;
		$db = null;
		$subject = "📥 insta42 : you received a comment"."\n";
		$link = "http://".$_SERVER['HTTP_HOST']."/insta42/"."comment.php?action=comment&id=$id";
